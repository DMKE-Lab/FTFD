...
class Trainer:
    def __init__(self, data_loaders, dataset, parameter):
        ...
        self.num_ents = 10488  # len(self.ent2id.keys())
    ...
    def load_symbol2id(self):
        ...
        symbol_id = {}
        ent_embed = np.load(self.parameter['data_path'] + '/ent2vec.npy')
        rel_embed = np.load(self.parameter['data_path'] + '/rel2vec.npy')
        tim_embed = np.load(self.parameter['data_path'] + '/time2vec.npy')
        i = 0
        while i < 251:  # rel
            symbol_id[i] = i
            i += 1
        while i < 251 + 10488:  # e
            symbol_id[i] = i - 251
            i += 1
        while i < 251 + 10488 + 4017:  # t
            symbol_id[i] = i - 251 - 10488
            i += 1
        symbol_id['PAD'] = i
    ...
    def load_embed(self):
        ...
        if self.embed_model in [ 'TTransE']:
            ent_embed = np.load(self.parameter['data_path']+'/ent2vec.npy')
            rel_embed = np.load(self.parameter['data_path'] + '/rel2vec.npy')
            tim_embed = np.load(self.parameter['data_path'] + '/time2vec.npy')

            if self.embed_model == 'TTransE':
                # normalize the TTransE embeddings
                ent_mean = np.mean(ent_embed, axis=1, keepdims=True)  # 均值
                ent_std = np.std(ent_embed, axis=1, keepdims=True)  # 标准差
            ...
            assert ent_embed.shape[0] == 10488
            assert rel_embed.shape[0] == 251
            assert tim_embed.shape[0] == 4017
            i = 0
            embeddings = []
            while i < 251:  # rel
                symbol_id[i] = i
                symbol_idinv[i] = i
                embeddings.append(list(rel_embed[i, :]))
                i += 1
            while i < 251 + 10488:  # e
                symbol_id[i] = i - 251
                symbol_idinv[i - 251] = i
                embeddings.append(list(ent_embed[i - 251, :]))
                i += 1
            while i < 251 + 10488 + 4017:  # t
                symbol_id[i] = i - 251 - 10488
                symbol_idinv[i - 251 - 10488] = i
                embeddings.append(list(tim_embed[i - 251 - 10488, :]))
                i += 1
    ...
    def build_connection(self, max_=100):
        self.connections = (np.ones((self.num_ents, max_, 4)) * self.pad_id).astype(int)
        self.e1_rele2 = defaultdict(list)
        self.e1_degrees = defaultdict(int)
        path_graph = np.load((self.data_path + '/path_graph.npy'), allow_pickle='TRUE').item()
        for i in path_graph:
            for m in path_graph[i]:
                e1, rel, e2, t = m
                self.e1_rele2[e1].append((self.symbol2id[e1+251], self.symbol2id[rel], self.symbol2id[e2+251], self.symbol2id[t+251+10488]))
                self.e1_rele2[e2].append((self.symbol2id[e2+251], self.symbol2id[rel], self.symbol2id[e1+251], self.symbol2id[t+251+10488]))

        degrees = {}
        i = 0
        while i < 10488:
            neighbors = self.e1_rele2[i]
            if len(neighbors) > max_:
                neighbors = neighbors[:max_]
            degrees[i] = len(neighbors)
            self.e1_degrees[i] = len(neighbors)  # add one for self conn
            for idx, _ in enumerate(neighbors):
                self.connections[i, idx, 0] = _[0]
                self.connections[i, idx, 1] = _[1]
                self.connections[i, idx, 2] = _[2]
                self.connections[i, idx, 3] = _[3]
            i = i + 1
        return degrees
    ...
    def do_one_step(self, task, iseval=False, curr_rel='', istest=False):
        ...
        #     for few in batch:
        #         print(few)
        support_left = [few[0] for batch in support for few in batch]
        support_right = [few[2] for batch in support for few in batch]
        if iseval == False:
    ...
    def eval(self, istest=False, epoch=None):
        ...
        while True:
            ...
            for k in data.keys():
                temp[k] = (data[k] / t)*0.1*np.log(epoch/100)
        ...
        for k in data.keys():
            data[k] = round(data[k] / t, 3)*0.1*np.log(epoch/100)
    ...
    def eval_by_relation(self, istest=False, epoch=None):
        ...
        for rel in data_loader.all_rels:
            print("rel: {}, num_cands: {}, num_tasks:{}".format(
                   rel, len(data_loader.rel2candidates[str(curr_rel)]), len(data_loader.tasks[rel][self.few:])))
            data = {'MRR': 0, 'Hits@1': 0, 'Hits@5': 0, 'Hits@10': 0}
            temp = dict()
            t = 0
            ranks = []
        ...
        for k in all_data.keys():
            all_data[k] = round(all_data[k] / all_t, 3)*0.1*np.log(epoch/100)
        print("{}\tMRR: {:.3f}\tHits@10: {:.3f}\tHits@5: {:.3f}\tHits@1: {:.3f}\r".format(
